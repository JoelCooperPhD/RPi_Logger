# Cameras Module Structure (Spec-Only)

Quick map of the Cameras spec files and their roles. Implementation is intentionally absent; each file documents intent and constraints.

## Top-Level Entry
- `rpi_logger/modules/Cameras/main_cameras.py` - CLI/options bootstrap, config resolution, stub supervisor wiring.
- `rpi_logger/modules/Cameras/bridge.py` - ModuleRuntime bridge for the stub stack (start/shutdown/commands/healthcheck).
...
## Tests (Spec Coverage)
- `rpi_logger/modules/Cameras/tests/modules/cameras/*` - test specs mirroring the runtime/storage/discovery/pipeline components (registry, preview/record pipelines, discovery, disk guard, retention, known cameras).
