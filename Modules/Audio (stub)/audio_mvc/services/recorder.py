"""Audio recording primitives built on sounddevice + wave."""

from __future__ import annotations

import asyncio
import logging
import threading
import time
import wave
from pathlib import Path
from typing import Dict, List, Optional

import numpy as np
import sounddevice as sd

from ..constants import AUDIO_BIT_DEPTH, AUDIO_CHANNELS_MONO
from ..level_meter import LevelMeter
from ..model import AudioDevice


class AudioDeviceRecorder:
    """Owns the sounddevice stream + sample buffering for one device."""

    def __init__(
        self,
        device: AudioDevice,
        sample_rate: int,
        level_meter: LevelMeter,
        logger: logging.Logger,
    ) -> None:
        self.device = device
        self.sample_rate = sample_rate
        self.level_meter = level_meter
        self.logger = logger.getChild(f"Dev{device.device_id}")
        self.stream: Optional[sd.InputStream] = None
        self.recording = False
        self._buffer: list[np.ndarray] = []
        self._buffer_lock = threading.Lock()
        self._last_status: Optional[str] = None

    # ------------------------------------------------------------------
    # Stream lifecycle

    def start_stream(self) -> None:
        if self.stream is not None:
            return

        def _callback(indata, frames, time_info, status):
            self._handle_callback(indata, status)

        channels = max(1, min(self.device.channels or 1, AUDIO_CHANNELS_MONO))
        try:
            stream = sd.InputStream(
                device=self.device.device_id,
                channels=channels,
                samplerate=self.sample_rate,
                dtype="float32",
                callback=_callback,
                blocksize=0,
            )
            stream.start()
            self.stream = stream
            self.logger.info("Input stream started (%d Hz)", self.sample_rate)
        except Exception as exc:
            self.stream = None
            self.logger.error("Failed to start stream: %s", exc)
            raise

    def stop_stream(self) -> None:
        stream = self.stream
        if stream is None:
            return
        self.stream = None
        try:
            stream.stop()
            stream.close()
        except Exception as exc:
            self.logger.debug("Stream close error: %s", exc)
        self.logger.info("Input stream stopped")

    # ------------------------------------------------------------------
    # Recording helpers

    def begin_recording(self) -> None:
        if self.recording:
            return
        with self._buffer_lock:
            self._buffer.clear()
        self.recording = True
        self.logger.info("Recording buffers prepared")

    def finish_recording(self, session_dir: Path, trial_number: int) -> Optional[Path]:
        if not self.recording:
            return None
        self.recording = False
        with self._buffer_lock:
            if not self._buffer:
                self.logger.warning("No audio data captured for device %d", self.device.device_id)
                return None
            audio_blocks = list(self._buffer)
            self._buffer.clear()

        audio_array = np.concatenate(audio_blocks)
        if audio_array.ndim > 1:
            audio_array = audio_array[:, 0]

        scaled = np.clip(audio_array, -1.0, 1.0)
        max_int = (2 ** (AUDIO_BIT_DEPTH - 1)) - 1
        int_samples = (scaled * max_int).astype(np.int16)

        safe_name = self.device.name.replace(" ", "-").replace(":", "").replace("_", "-").lower()
        session_name = session_dir.name
        if "_" in session_name:
            timestamp = session_name.split("_", 1)[1]
        else:
            timestamp = time.strftime("%Y%m%d_%H%M%S")
        filename = f"{timestamp}_AUDIO_trial{trial_number:03d}_MIC{self.device.device_id}_{safe_name}.wav"
        file_path = session_dir / filename

        try:
            with wave.open(str(file_path), "wb") as handle:
                handle.setnchannels(1)
                handle.setsampwidth(AUDIO_BIT_DEPTH // 8)
                handle.setframerate(self.sample_rate)
                handle.writeframes(int_samples.tobytes())
        except Exception as exc:
            self.logger.error("Failed to write wav file: %s", exc)
            return None

        self.logger.info("Saved audio file: %s", file_path.name)
        return file_path

    # ------------------------------------------------------------------
    # Audio callback

    def _handle_callback(self, indata, status: sd.CallbackFlags) -> None:
        mono = indata[:, 0] if indata.ndim > 1 else indata
        now = time.time()
        try:
            self.level_meter.add_samples(mono.tolist(), now)
        except Exception:
            pass

        if self.recording:
            with self._buffer_lock:
                self._buffer.append(mono.copy())

        if status:
            status_str = str(status)
            if status_str != self._last_status:
                self.logger.warning("Audio callback status: %s", status_str)
                self._last_status = status_str


class RecorderService:
    """Manage AudioDeviceRecorder instances keyed by device id."""

    def __init__(
        self,
        logger: logging.Logger,
        sample_rate: int,
        start_timeout: float,
        stop_timeout: float,
    ) -> None:
        self.logger = logger.getChild("RecorderService")
        self.sample_rate = sample_rate
        self.start_timeout = start_timeout
        self.stop_timeout = stop_timeout
        self.recorders: Dict[int, AudioDeviceRecorder] = {}

    async def enable_device(self, device: AudioDevice, meter: LevelMeter) -> bool:
        recorder = self.recorders.get(device.device_id)
        if recorder is None:
            recorder = AudioDeviceRecorder(device, self.sample_rate, meter, self.logger)
            self.recorders[device.device_id] = recorder

        try:
            await asyncio.wait_for(
                asyncio.to_thread(recorder.start_stream),
                timeout=self.start_timeout,
            )
            return True
        except asyncio.TimeoutError:
            self.logger.error("Timeout starting device %d", device.device_id)
        except Exception as exc:
            self.logger.error("Failed to start device %d: %s", device.device_id, exc)
        self.recorders.pop(device.device_id, None)
        return False

    async def disable_device(self, device_id: int) -> None:
        recorder = self.recorders.pop(device_id, None)
        if not recorder:
            return
        try:
            await asyncio.wait_for(
                asyncio.to_thread(recorder.stop_stream),
                timeout=self.stop_timeout,
            )
        except Exception as exc:
            self.logger.debug("Device %d stop raised: %s", device_id, exc)

    async def begin_recording(self, device_ids: List[int]) -> int:
        started = 0
        for device_id in device_ids:
            recorder = self.recorders.get(device_id)
            if not recorder:
                continue
            try:
                recorder.begin_recording()
                started += 1
            except Exception as exc:
                self.logger.error("Failed to prepare recorder %d: %s", device_id, exc)
        return started

    async def finish_recording(self, session_dir: Path, trial_number: int) -> List[Path]:
        results: List[Path] = []
        tasks = []
        for device_id, recorder in list(self.recorders.items()):
            tasks.append(asyncio.to_thread(recorder.finish_recording, session_dir, trial_number))

        if not tasks:
            return results

        finished = await asyncio.gather(*tasks, return_exceptions=True)
        for maybe_path in finished:
            if isinstance(maybe_path, Exception) or maybe_path is None:
                continue
            results.append(maybe_path)
        return results

    async def stop_all(self) -> None:
        tasks = [self.disable_device(device_id) for device_id in list(self.recorders.keys())]
        if tasks:
            await asyncio.gather(*tasks, return_exceptions=True)
        self.recorders.clear()
