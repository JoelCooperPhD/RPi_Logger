"""EyeT (stub) module entry point leveraging the stub (codex) VMC stack."""
...
DISPLAY_NAME = "EyeT (Stub)"
MODULE_ID = "eyet_stub"
DEFAULT_OUTPUT_SUBDIR = Path("eye-tracker-stub")
...
def parse_args(argv: Optional[list[str]] = None):
    ...
    parser.add_argument(
        ...
        help="Stub preview refresh rate in Hz",
    )
    ...
    args.window_geometry = load_window_geometry_from_config(config, getattr(args, "window_geometry", None))
    args.config = config
...
def build_runtime(context):
    return EyeTStubRuntime(context)
...
async def main(argv: Optional[list[str]] = None) -> None:
    ...
    if not getattr(args, "enable_commands", False):
        logger.error("EyeT (stub) module must be launched by the logger controller.")
        return
