"""Base camera settings window: resolution/FPS and live controls.

This module provides a base class for camera settings windows that can be
subclassed by USB (Cameras) and CSI (CSICameras) modules.
"""
...
if TYPE_CHECKING:
    from rpi_logger.modules.base.camera_types import CameraCapabilities, ControlInfo, ControlType

# Base default settings (subclasses may extend)
...
class CameraSettingsWindowBase:
    """Base class for camera settings windows with resolution/FPS and camera controls.

    Subclasses should override these class attributes:
    - IMAGE_CONTROLS: List of image adjustment control names
    - EXPOSURE_FOCUS_CONTROLS: List of exposure/focus control names
    - CONTROL_DEPENDENCIES: Dict mapping child -> (parent, enable_condition)
    - WINDOW_TITLE_PREFIX: String prefix for window title
    ...
    IMAGE_CONTROLS: List[str] = []
    EXPOSURE_FOCUS_CONTROLS: List[str] = []
    CONTROL_DEPENDENCIES: Dict[str, tuple] = {}
    WINDOW_TITLE_PREFIX: str = "Camera Settings"
    ...
    def _get_sensor_info_dialog():
        """Return (show_sensor_info_func, has_dialog) tuple. Override in subclass."""
        return None, False
    ...
    def set_active_camera(self, camera_id: Optional[str]) -> None:
        ...
        if camera_id:
            self._latest.setdefault(camera_id, dict(self.DEFAULT_SETTINGS))
    ...
    def update_camera_capabilities(self, camera_id: str, capabilities: "CameraCapabilities", *,
        ...
        # Format backend using subclass mapping
        backend_display = self.BACKEND_DISPLAY_MAP.get((backend or "").lower(), backend or "Unknown")
    ...
    def get_latest_settings(self, camera_id: str) -> Dict[str, str]:
        return self._latest.get(camera_id, dict(self.DEFAULT_SETTINGS))
    ...
    def _ensure_window(self) -> None:
        ...
        self._window.title(self.WINDOW_TITLE_PREFIX)
    ...
    def _build_capture_card(self, parent, row: int) -> None:
        ...
        # Audio recording checkbox (only if SUPPORTS_AUDIO)
        next_row = 2
        ...
        btn_frame.grid(row=next_row, column=0, columnspan=4, sticky="e", pady=(6, 0))
    ...
    def _build_footer(self, parent, row: int) -> None:
        """Build the footer with camera info and action buttons."""
        bg = Colors.BG_DARK if HAS_THEME and Colors else "#2b2b2b"
        fg = Colors.FG_SECONDARY if HAS_THEME and Colors else "#95a5a6"
    ...
    def _rebuild_controls_section(self) -> None:
        ...
        image_controls = []
        seen = set()
        for name in self.IMAGE_CONTROLS:
        ...
        exposure_controls = []
        for name in self.EXPOSURE_FOCUS_CONTROLS:
            if name in caps.controls and name not in seen:
    ...
    def _build_control_widget(self, parent, row: int, name: str, ctrl: "ControlInfo") -> None:
        ...
        from rpi_logger.modules.base.camera_types import ControlType
    ...
    def _format_control_name(self, name: str) -> str:
        """Format control name for display (insert spaces before capitals)."""
        result = []
    ...
    def _update_dependent_control_states(self) -> None:
        ...
        for child_name, (parent_name, is_enabled_func) in self.CONTROL_DEPENDENCIES.items():
    ...
    def _on_control_changed(self, name: str) -> None:
        ...
        try:
            if ctrl.control_type == ControlType.BOOLEAN:
                ...
                value = self._parse_enum_value(var.get())
            else:
                value = var.get()
    ...
    def _parse_enum_value(self, raw_value: str) -> Any:
        """Parse enum value. Override in subclass for module-specific format."""
        return raw_value
    ...
    def _update_audio_checkbox_visibility(self) -> None:
        ...
        if not self.SUPPORTS_AUDIO or not self._record_audio_frame:
            return
        try:
            has_audio = self._has_audio_sibling.get(self._active_camera or "", False)
            if has_audio:
                self._record_audio_frame.grid()
            else:
                self._record_audio_frame.grid_remove()
        except Exception:
    ...
    def _clamp_settings_to_options(self, camera_id: str) -> None:
        ...
        latest = self._latest.setdefault(camera_id, dict(self.DEFAULT_SETTINGS))
