...
class AudioState:
    ...
    def snapshot(self) -> AudioSnapshot:
        return AudioSnapshot(self.device, self.level_meter, self.recording,
                             self.trial_number, self.session_dir, self._status_text)

    def set_device(self, device: AudioDeviceInfo) -> None:
        self.device, self.level_meter = device, LevelMeter()
        self._update_status()
        self._notify()

    def clear_device(self) -> None:
        self.device = self.level_meter = None
        self._update_status()
        self._notify()
    ...
    def _update_status(self) -> None:
        if self.recording:
            self._status_text = f"Recording trial {self.trial_number} ({self.device.name if self.device else 'unknown'})"
    ...
    def status_payload(self) -> dict[str, object]:
        return {"recording": self.recording, "trial_number": self.trial_number,
    ...
    def get_persistable_state(self) -> dict[str, Any]:
        return {"device_name": self.device.name if self.device else ""}
