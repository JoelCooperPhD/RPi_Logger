"""CSI camera runtime using Elm/Redux architecture internally.
...
class CSICamerasRuntime(ModuleRuntime):
    ...
    async def start(self) -> None:
        ...
        if self.ctx.view:
            ...
            if hasattr(self.ctx.view, "set_data_subdir"):
                self.ctx.view.set_data_subdir("Cameras_CSI")
        ...
        self.logger.info("CSI Cameras runtime ready")
    ...
    def _save_settings(self, settings: CameraSettings) -> None:
        asyncio.create_task(self._persist_settings(settings))

    async def _persist_settings(self, settings: CameraSettings) -> None:
