...
from rpi_logger.modules.Cameras2.bridge import Cameras2Runtime
from rpi_logger.modules.Cameras2.runtime import CameraDescriptor, CameraId, CapabilityMode
...
def _make_context(tmp_path):
    return types.SimpleNamespace(
        ...
        module_dir=tmp_path,
        logger=logging.getLogger("Cameras2Test"),
...
async def test_runtime_refreshes_and_attaches_multiple_cameras(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "rpi_logger.modules.Cameras2.bridge.discover_usb_devices",
        lambda logger=None: descriptors,
    )
    monkeypatch.setattr("rpi_logger.modules.Cameras2.bridge.discover_picam", lambda logger=None: [])
    ...
    monkeypatch.setattr(Cameras2Runtime, "_probe_capabilities", fake_probe, raising=False)
    monkeypatch.setattr(Cameras2Runtime, "_open_backend", fake_open, raising=False)

    ctx = _make_context(tmp_path)
    runtime = Cameras2Runtime(ctx)
