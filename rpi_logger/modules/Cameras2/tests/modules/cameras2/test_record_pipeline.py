...
from rpi_logger.modules.Cameras2.storage import DiskGuard, resolve_session_paths
...
class DummyRecorder:
    ...
    async def enqueue(self, handle, frame, *, timestamp):
        self.enqueued += 1
...
async def test_recorder_uses_target_fps(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("rpi_logger.modules.Cameras2.runtime.record.recorder.cv2.VideoWriter", FakeWriter)

    recorder = Recorder(queue_size=2)
