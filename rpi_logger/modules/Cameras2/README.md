# Cameras2 Module Specification

This document describes the intended behavior, threading/async constraints, and component boundaries for the Cameras2 module (stub/codex-based). All text only; no executable code committed yet.
...
- Cameras2 runs under the stub VMC stack used by `main_stub_codex.py`: use `StubCodexSupervisor`, `StubCodexModel`, and `StubCodexController` wiring patterns (ModuleRuntime/RuntimeContext, lifecycle hooks, retry policies).
- CLI/options should follow stub conventions (`mode`, `output-dir`, `session-prefix`, `enable-commands`, `window-geometry`, `log-level`, `log-file`), with Cameras2-specific flags layered on top.
...
- Commands/user actions must honor the stub verbs (`start_recording`/`stop_recording`/`quit`/`get_status`/`start_session`/`stop_session`) and forward unknowns to the Cameras2 runtime as needed.
...
- Logging should attach through stub view hooks when GUI exists; keep loggers child-named for integration (`logger.getChild("Cameras2")` etc.).
