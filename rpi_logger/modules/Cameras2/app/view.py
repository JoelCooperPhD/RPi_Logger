"""Tk-backed view for Cameras2 using the stub view's content hooks."""
...
from rpi_logger.modules.Cameras2.app.views.adapter import ViewAdapter
from rpi_logger.modules.Cameras2.app.widgets.settings_panel import DEFAULT_SETTINGS, SettingsWindow
from rpi_logger.modules.Cameras2.app.views.metrics_panel import MetricsWindow


class Cameras2View:
    """Composes the Cameras2 Tk UI (tabs, metrics, settings)."""
    ...
    def __init__(self, stub_view: Any = None, *, logger: LoggerLike = None) -> None:
        ...
        self._metrics_window: Optional[MetricsWindow] = None
        self._metrics_toggle_var: Optional[Any] = None
    ...
    def attach(self) -> None:
        """Mount the Cameras2 UI inside the stub view frame, if available."""

        if not self._stub_view:
            self._logger.info("Cameras2 view running headless (stub view missing)")
            return
        ...
        try:
            ...
            self._logger.warning("Tk unavailable for Cameras2 view: %s", exc)
            return
        self._ui_thread = threading.current_thread()
        ...
        self._has_ui = True
        self._logger.info("Cameras2 view attached")
    ...
    def _build_layout(self, parent, tk, ttk) -> None:
        ...
        self._install_metrics_window(tk)
    ...
    def remove_camera(self, camera_id: str) -> None:
        ...
        if self._metrics_window:
            self._metrics_window.remove_camera(camera_id)
        if self._settings_window:
    ...
    def _install_metrics_window(self, tk) -> None:
        if self._metrics_toggle_var is None:
            self._metrics_toggle_var = tk.BooleanVar(master=self._root, value=False)
        if self._metrics_window is None:
            self._metrics_window = MetricsWindow(self._root, logger=self._logger)
            self._metrics_window.bind_toggle_var(self._metrics_toggle_var)

    def _toggle_metrics_window(self) -> None:
        if not self._metrics_window or not self._metrics_toggle_var:
            return
        visible = bool(self._metrics_toggle_var.get())
        if visible:
            self._metrics_window.show()
            self._metrics_window.set_active_camera(self._active_camera_id)
            for key, payload in self._latest_metrics.items():
                self._metrics_window.update_metrics(key, payload)
        else:
            self._metrics_window.hide()

    def _apply_metrics_update(self, camera_id: str) -> None:
        if self._metrics_window:
            self._metrics_window.update_metrics(camera_id, self._latest_metrics.get(camera_id, {}))
        self._update_io_stub_line()
    ...
    def _install_settings_menu(self, tk) -> None:
        ...
        if self._metrics_toggle_var is not None:
            menu.add_checkbutton(
                label="Show Metrics Window",
                variable=self._metrics_toggle_var,
                command=self._toggle_metrics_window,
            )
