"""Per-camera slot state used by the USB Cameras controller."""
...
from ..domain.pipelines import USBCapturePipeline
...
class USBCameraSlot:
    ...
    image_pipeline: Optional[USBCapturePipeline] = field(default=None, repr=False)
    ...
    def attach_queues(
        ...
        *,
        ...
        logger.debug(
            "USBCameraSlot queues attached | index=%s capture=%s preview=%s storage=%s",
            self.index,
            bool(capture_queue),
            bool(preview_queue),
            bool(storage_queue),
    ...
    def reset_pipeline(self) -> None:
        ...
        logger.debug("USBCameraSlot pipeline reset | index=%s", self.index)


__all__ = ["USBCameraSlot"]
