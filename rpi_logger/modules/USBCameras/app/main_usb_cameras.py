"""USB Cameras module entry point using the stub (codex) VMC stack."""
...
if __package__ in {None, ""}:
    __package__ = "rpi_logger.modules.USBCameras"
...
MODULE_ID = "usb_cameras"
DEFAULT_OUTPUT_SUBDIR = Path("usb_cameras")
...
def parse_args(argv: Optional[list[str]] = None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Maximum number of USB cameras to preview",
    )
    parser.add_argument(
        "--device-indices",
        type=str,
        default=None,
        help="Comma-separated list of /dev/video indices to use (default: auto-discover)",
    )
    parser.add_argument(
        "--capture-width",
        type=int,
        default=None,
        help="Main stream width in pixels (default: device native)",
    )
    parser.add_argument(
        "--capture-height",
        type=int,
        default=None,
        help="Main stream height in pixels (default: device native)",
    )
    parser.add_argument(
        "--target-fps",
        type=float,
        default=None,
        help="Requested capture FPS (default: device default)",
    )
    parser.add_argument(
        "--save-images",
        action="store_true",
...
def build_runtime(context):
    return USBCamerasRuntime(context)
...
async def main(argv: Optional[list[str]] = None) -> None:
    ...
    if not args.enable_commands:
        logger.error("%s module must be launched by the logger controller.", DISPLAY_NAME)
        return
