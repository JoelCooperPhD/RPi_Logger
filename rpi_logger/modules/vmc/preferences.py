"""Standalone preference helpers for the stub (codex) stack.

These mirror the lightweight API provided by ``rpi_logger.modules.base.preferences``
so that the stub module can operate independently while still exposing the same
contract to runtimes built on top of it.
"""
...
class StubPreferences:
    """Minimal preference wrapper around :class:`ConfigManager`.

    The interface matches ``ModulePreferences`` from ``modules.base`` so other modules
    can consume ``model.preferences`` without caring about the implementation source.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._cache: Dict[str, Any] = dict(initial_data) if initial_data else {}
        if not initial_data:
            self.reload()
...
class ScopedPreferences:
    """Wrapper around :class:`StubPreferences` that automatically prefixes keys."""

    def __init__(self, base: StubPreferences, prefix: str, *, separator: str = ".") -> None:
        self._base = base
...
# Alias mirrors the base module name so consumers can keep using the same name.
ModulePreferences = StubPreferences
