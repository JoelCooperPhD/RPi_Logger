"""
VOG Connection Manager

Manages USB and XBee device detection and connections for all VOG device types.
...
DeviceCallback = Callable[[str, VOGDeviceType, VOGHandler], Awaitable[None]]
DisconnectCallback = Callable[[str, VOGDeviceType], Awaitable[None]]


class ConnectionManager:
    """
    Manages USB and XBee device detection and connections for VOG devices.

    Features:
    - Continuous scanning for multiple VOG device types
    - Automatic handler creation based on device type
    - Hot-plug detection (connect/disconnect)
    - XBee network management for wireless wVOG devices
    - Mutual exclusion between USB wVOG and XBee
    """
    ...
    def __init__(
        ...
        Args:
            ...
            system: Optional VOGSystem reference for handlers
        """
        ...
        # State tracking
        self._handlers: Dict[str, VOGHandler] = {}
        self._device_types: Dict[str, VOGDeviceType] = {}
        self._known_ports: Set[str] = set()
        self._running = False
        self._scan_task: Optional[asyncio.Task] = None
        self._xbee_start_task: Optional[asyncio.Task] = None
    ...
    @property
    def handlers(self) -> Dict[str, VOGHandler]:
        """Return all active handlers."""
        return self._handlers.copy()

    @property
    def connected_devices(self) -> Dict[str, VOGDeviceType]:
        """Return dict of connected device ports to their types."""
        return self._device_types.copy()

    @property
    def has_usb_wvog(self) -> bool:
        """Check if any USB wVOG is connected."""
        return VOGDeviceType.WVOG_USB in self._device_types.values()
    ...
    async def start(self) -> None:
        ...
        self._running = True
        logger.info("Starting VOG connection manager")
        ...
        # Start XBee manager if enabled and no USB wVOG connected
        if self._xbee_manager and not self.has_usb_wvog:
    ...
    async def stop(self) -> None:
        ...
        logger.info("VOG connection manager stopped")
    ...
    async def _scan_ports(self) -> None:
        """Scan for VOG devices and handle connect/disconnect events."""
        try:
            # Get current ports
    ...
    async def _check_handler_health(self) -> None:
        ...
        Disconnects handlers that have stopped unexpectedly.
        """
        unhealthy_devices = []
    ...
    async def _handle_connect(
        ...
        Args:
            ...
            device_type: Type of VOG device
        ...
        # Check mutual exclusion if wVOG USB connected
        if device_type == VOGDeviceType.WVOG_USB:
    ...
    def _create_handler(
        ...
        elif device_type in (VOGDeviceType.WVOG_USB, VOGDeviceType.WVOG_WIRELESS):
            protocol = WVOGProtocol()
        else:
            logger.warning(f"Unknown device type: {device_type}")
            return None

        # Create handler using transport's write/read methods
        # We need to create a wrapper that looks like USBSerialDevice
        from .transport_device_adapter import TransportDeviceAdapter
    ...
    async def _handle_disconnect(self, port: str) -> None:
        ...
        if handler:
            ...
            # Check mutual exclusion if wVOG USB disconnected
            if device_type == VOGDeviceType.WVOG_USB:
                await self._check_mutual_exclusion()
    ...
    async def _on_xbee_device_discovered(
        ...
        """
        Handle a wireless wVOG device discovered via XBee.

        Args:
            node_id: XBee node ID (e.g., "wVOG_01")
            transport: XBee transport for the device
        """
        logger.info(f"Wireless wVOG discovered: {node_id}")

        # Create handler for wireless device using transport adapter
        from .transport_device_adapter import TransportDeviceAdapter
        device_adapter = TransportDeviceAdapter(transport, node_id)
        ...
        self._device_types[node_id] = VOGDeviceType.WVOG_WIRELESS

        # Notify callback
        if self.on_device_connected:
            await self.on_device_connected(
                node_id,
                VOGDeviceType.WVOG_WIRELESS,
                handler
            )

    async def _on_xbee_device_lost(self, node_id: str) -> None:
        """
        Handle a wireless wVOG device being lost.
        ...
        if handler:
            logger.info(f"Wireless wVOG lost: {node_id}")
            await handler.stop()
    ...
    async def _on_xbee_status_change(self, status: str, detail: str) -> None:
        ...
        """
        logger.info(f"XBee status change: {status} {detail}")
    ...
    async def _check_mutual_exclusion(self) -> None:
        """
        Check and enforce mutual exclusion between USB wVOG and XBee.

        When USB wVOG is connected, XBee should be disabled.
        When USB wVOG is disconnected, XBee can be re-enabled.
        """
        if not self._xbee_manager:
            return

        if self.has_usb_wvog:
            # Disable XBee when USB wVOG is connected
            ...
            # Re-enable XBee when no USB wVOG
    ...
    def get_handler(self, port: str) -> Optional[VOGHandler]:
        """
